...
(function(jQuery) {
    function Plot(target_, data_, options_) {
        ...
        function calculateTicks(axis, axisOptions) {
            ...
            if (axisOptions.ticks) {
                ...
                for (i = 0; i < ticks.length; ++i) {
                    ...
                    if (typeof(t) == "object") {
                        v = t[0];
                        //FIX FOR NOT SHOWING LABELS ON ZOOMED GRAPHS
                        if (v < axis.min || v > axis.max) {
                          axis.ticks[i] = { v: axis.min, label: "" };
                          continue;
                        }                          
                        if (t.length > 1)
                            label = t[1];
        ...
        function calculateSpacing() {
            ...
            // measure it
            var dummyDiv = jQuery('<div style="position:absolute;top:-10000px;font-size:smaller" class="gridLabel">' + max_label + '</div>').appendTo(target);
        ...
        function fillInSeriesOptions() {
            ...
            for (i = 0; i < series.length; ++i) {
                ...
                // copy the rest
                s.lines = jQuery.extend(true, {}, options.lines, s.lines);
                s.points = jQuery.extend(true, {}, options.points, s.points);
                s.bars = jQuery.extend(true, {}, options.bars, s.bars);
        ...
        function insertLegend() {
            ...
            if (fragments.length > 0) {
                ...
                if (options.legend.container != null)
                    ...
                    var div = jQuery('<div class="legend" style="position:absolute;z-index:2;' + pos +'">' + table + '</div>').appendTo(target);
        ...
        function onMouseDown(e) {
            ...
            jQuery(document).one("mouseup", onSelectionMouseUp);
        }
        ...
        function onClick(e) {
            ...
            var offset = jQuery(overlay).offset();
            var pos = {};
    ...
    jQuery.plot = function(target, data, options) {
