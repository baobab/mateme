class AncConnection::PersonNameCode < ActiveRecord::Base
  ...
  belongs_to :person_name, :class_name => "AncConnection::PersonName", :conditions => {:voided => 0}
  
  def self.rebuild_person_name_codes
    AncConnection::PersonNameCode.delete_all
    names = AncConnection::PersonName.find(:all)
    names.each {|name|
      AncConnection::PersonNameCode.create(
