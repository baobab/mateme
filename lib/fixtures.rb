...
class ActiveRecord::Base
  ...
  def self.to_fixtures(path=nil)
    path ||= File.expand_path("db/data/defaults/#{table_name}.yml", RAILS_ROOT)
  ...
  def to_fixture_name  
    n = self.fixture_names[self.class.table_name].map{|att| "#{self.instance_eval(att)}"}.join("_") unless self.fixture_names.blank? || self.fixture_names[self.class.table_name].blank?
